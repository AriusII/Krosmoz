...
public class AllianceListMessage : DofusMessage
{
	public new const uint StaticProtocolId = 6408;

	public override uint ProtocolId =>
		StaticProtocolId;

	public static AllianceListMessage Empty =>
		new() { Alliances = [] };

	public required IEnumerable<AllianceFactSheetInformations> Alliances { get; set; }
	...
	public override void Deserialize(BigEndianReader reader)
	{
		...
		var alliances = new AllianceFactSheetInformations[alliancesCount];
		for (var i = 0; i < alliancesCount; i++)
		{
			var entry = AllianceFactSheetInformations.Empty;
