package gs
{
   ...
   public class TweenLite
   {
      ...
      {
         var _loc3_:Number = Number(param1.target.progress);
      ...
      public function initTweenVals(param1:Boolean = false, param2:String = "") : void
      {
         ...
         if(!param1 && Boolean(overwriteManager.enabled))
         {
         ...
         else
         {
            ...
            for(_loc3_ in this._specialVars)
            {
               ...
               {
                  if(!(this._isDisplayObject && (_loc3_ == "tint" || _loc3_ == "removeTint" || _loc3_ == "frame")) && !(_loc3_ == "volume" && Boolean(this.target.hasOwnProperty("soundTransform"))))
                  {
