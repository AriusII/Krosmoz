package com.ankamagames.dofus.misc.utils
{
   ...
   public class LoadingScreen extends UiRootContainer implements FinalizableUIComponent, IResourceObserver
   {
      ...
      public function LoadingScreen(param1:Boolean = false, param2:Boolean = false)
      {
         ...
         if(!beforeLogin && Kernel.getWorker() && Boolean(ConnectionsHandler.getConnection()))
         {
            EnterFrameDispatcher.addEventListener(this.onEnterFrame,"loadingScreen");
            addEventListener(Event.REMOVED_FROM_STAGE,this.onRemoveFromStage);
         }
         loaderDi = BuildInfos.BUILD_TYPE == BuildTypeEnum.DEBUG || BuildInfos.BUILD_TYPE == BuildTypeEnum.INTERNAL;
         if(!loaderDi)
         {
            this._customLoadingScreen = CustomLoadingScreenManager.getInstance().currentLoadingScreen;
         }
         if(Boolean(this._customLoadingScreen) && this._customLoadingScreen.canBeReadOnScreen(beforeLogin))
         {
            try
            {
      ...
      private function finalizeInitialization() : void
      {
         ...
         if(Boolean(this._customLoadingScreen) && Boolean(this._customLoadingScreen.linkUrl))
         {
      ...
      private function displayAchievmentProgressBar(param1:AchievementListMessage) : void
      {
         ...
         this._tipsBackgroundBitmap.y -= 18;
         this._tipsBackgroundBitmap.height -= 200;
         this._tipsTextField.y = this._tipsBackgroundBitmap.y + 10;
      ...
      {
         if(this._customLoadingScreen && this._customLoadingScreen.canBeReadOnScreen(this._beforeLogin) && Boolean(this._customLoadingScreen.linkUrl))
         {
      ...
      public function onEnterFrame(param1:Event) : void
      {
         ...
         var _loc3_:Array = ConnectionsHandler.getConnection().pauseBuffer;
