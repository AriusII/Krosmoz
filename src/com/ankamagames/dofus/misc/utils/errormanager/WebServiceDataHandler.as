package com.ankamagames.dofus.misc.utils.errormanager
{
   ...
   public class WebServiceDataHandler extends EventDispatcher
   {
      ...
      private var _log:Logger = Log.getLogger(getQualifiedClassName(WebServiceDataHandler));
      
      private var _exceptionsList:Vector.<DataExceptionModel> = new Vector.<DataExceptionModel>();
      
      private var _webService:RpcServiceManager;
      
      private var _exceptionsInProgress:Dictionary = new Dictionary(true);
      
      private var _timersList:Dictionary = new Dictionary(true);
      ...
      public function createNewException(param1:Object, param2:String) : DataExceptionModel
      {
         ...
         for each(_loc5_ in Kernel.getWorker().framesList)
         {
            ...
            _loc6_ += String(!!_loc7_[1] ? _loc7_[1] : _loc7_[0]).replace("Frame","") + ",";
         }
      ...
      public function cleanStacktrace(param1:String) : String
      {
         ...
         {
            _loc5_ = int(_loc3_.indexOf(_loc4_));
            if(_loc5_ > 0)
            {
               _loc4_ = _loc4_.replace(/\\/g,"/");
            }
            _loc6_ = /^(.*?\[)(.*?)((\/modules\/Ankama_|\/com\/ankama).*?)(:?[0-9]*?)(\].*?)/g;
            _loc2_ += _loc4_.replace(_loc6_,"$1$3$6");
