package com.ankamagames.dofus.misc.utils.errormanager
{
   ...
   public class DofusErrorHandler
   {
      ...
      {
         return Boolean(_manualActivation.data) && Boolean(_manualActivation.data.force);
      }
      ...
      {
         if(Boolean(File.applicationDirectory.resolvePath("debug").exists) || Boolean(File.applicationDirectory.resolvePath("debug.txt").exists) || _manualActivation.data && _manualActivation.data.force)
         {
      ...
      private function removeDebugFile() : void
      {
         ...
         try
         {
            ...
            if(Boolean(debugFile) && Boolean(debugFile.exists))
            {
      ...
      private function onError(param1:ErrorReportedEvent) : void
      {
         ...
         reportInfo = this.getReportInfo(error,e.text);
         if(reportInfo != null)
         {
         ...
         try
         {
            ...
            if(_logBuffer)
            {
               ...
               for each(log in logs)
               {
                  ...
                  {
                     htmlBuffer += "\t\t\t<li class=\"l_" + log.level + "\">" + log.message + "</li>\n";
                  }
               }
               o.logSos = htmlBuffer;
            }
            ...
            while(cellId < AtouinConstants.MAP_CELLS_COUNT)
            {
               ...
               if(Boolean(entityInfoProvider) && Boolean(entitiesOnCell.length))
               {
