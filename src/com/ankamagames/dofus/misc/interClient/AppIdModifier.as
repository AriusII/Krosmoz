package com.ankamagames.dofus.misc.interClient
{
   ...
   public class AppIdModifier
   {
      ...
      public function AppIdModifier()
      {
         ...
         fs.close();
         startIdTagPos = int(content.indexOf("<" + APP_ID_TAG + ">"));
         endIdTagPos = int(content.indexOf("</" + APP_ID_TAG + ">"));
         if(startIdTagPos == -1 || endIdTagPos == -1)
         {
            return;
         }
         startIdTagPos = startIdTagPos + 2 + APP_ID_TAG.length;
         currentId = content.substr(startIdTagPos,endIdTagPos - startIdTagPos);
         tmp = currentId.split("-");
         if(!tmp[1])
         {
         ...
         while(true)
         {
            ...
            try
            {
               ...
               ts = Number(idFileStream.readDouble());
               idFileStream.close();
