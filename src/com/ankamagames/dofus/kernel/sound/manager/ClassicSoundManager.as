package com.ankamagames.dofus.kernel.sound.manager
{
   ...
   public class ClassicSoundManager extends EventDispatcher implements ISoundManager
   {
      ...
      public function handleFLAEvent(param1:String, param2:String, param3:String, param4:Object = null) : void
      {
         ...
         var _loc5_:Number = Number(param4.absoluteBounds.x);
         var _loc6_:Number = Number(param4.absoluteBounds.y);
         var _loc7_:int = int(param4.id);
         ...
         if(_loc17_ is LocalizedSound && Boolean(_loc10_.rollOff))
         {
            _loc14_ = this._rollOffPresets[_loc10_.rollOff];
            if((Boolean(_loc14_)) && _loc17_ is LocalizedSound)
            {
      ...
      private function findXmlSoundsInDirectory(param1:File) : void
      {
         ...
         if(param1.exists)
         {
            ...
            {
               if(Boolean(_loc3_.isDirectory) && _loc3_.name != ".svn" && _loc3_.name != "presets")
               {
                  this.findXmlSoundsInDirectory(_loc3_);
               }
               else if(Boolean(_loc3_.extension) && _loc3_.extension.toUpperCase() == "XML")
               {
      ...
      private function onXMLSoundFileLoaded(param1:ResourceLoadedEvent) : void
      {
         ...
         var _loc4_:int = int(this._XMLSoundFilesToLoad.indexOf(param1.uri));
         if(_loc4_ >= 0)
         {
