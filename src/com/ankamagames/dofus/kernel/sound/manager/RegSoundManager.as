package com.ankamagames.dofus.kernel.sound.manager
{
   ...
   public class RegSoundManager extends EventDispatcher implements ISoundManager
   {
      ...
      public function setSubArea(param1:Map = null) : void
      {
         ...
         var _loc8_:AmbientSound = null;
         var _loc9_:AmbientSound = null;
         ...
         if(_loc2_)
         {
            for each(_loc7_ in _loc2_.sounds)
            {
               _loc8_ = new AmbientSound();
               _loc8_.channel = _loc7_.channel;
               _loc8_.criterionId = _loc7_.criterionId;
               _loc8_.id = _loc7_.id;
               _loc8_.silenceMax = _loc7_.silenceMax;
               _loc8_.silenceMin = _loc7_.silenceMin;
               _loc8_.volume = _loc7_.volume;
               _loc4_[_loc7_.type_id - 1].push(_loc8_);
            }
         }
         for each(_loc6_ in _loc3_.ambientSounds)
         {
            _loc9_ = new AmbientSound();
            _loc9_.channel = _loc6_.channel;
            _loc9_.criterionId = _loc6_.criterionId;
            _loc9_.id = _loc6_.id;
            _loc9_.silenceMax = _loc6_.silenceMax;
            _loc9_.silenceMin = _loc6_.silenceMin;
            _loc9_.volume = _loc6_.volume;
            _loc10_ = true;
            if(_loc10_)
            {
               _loc4_[_loc6_.type_id - 1].push(_loc9_);
            }
         }
         _log.info("Subarea Id : " + _loc3_.id + " / Map id : " + param1.id);
         this._ambientManager.setAmbientSounds(_loc4_[0],_loc4_[1]);
      ...
      public function handleFLAEvent(param1:String, param2:String, param3:String, param4:Object = null) : void
      {
         ...
         if(param4 is TiphonSprite && TiphonSprite(param4).parentSprite && Boolean(TiphonSprite(param4).parentSprite.getSubEntitySlot(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_PET,0)))
         {
            return;
         }
         var _loc5_:Number = 0;
         ...
         {
            _loc5_ = Number(param4.absoluteBounds.x);
            _loc6_ = Number(param4.absoluteBounds.y);
            _loc7_ = int(param4.id);
            if(_loc7_ != PlayedCharacterManager.getInstance().infos.id && _loc7_ > 0 && Kernel.getWorker().getFrame(FightBattleFrame) == null)
            {
               return;
            }
         }
         ...
         else
         {
            ...
            if(_loc9_.parentSprite.getSubEntitySlot(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_MOUNT_DRIVER,0) != null)
            {
               ...
               {
                  _loc5_ = Number(_loc10_.absoluteBounds.x);
                  _loc6_ = Number(_loc10_.absoluteBounds.y);
         ...
         {
            _loc8_ = int(TiphonEntityLook(param4.look).firstSkin);
         }
         if(param3)
         {
      ...
      public function removeSoundEntity(param1:ISound) : void
      {
         ...
         var _loc2_:int = int(this._reverseEntitySounds[param1]);
      ...
      public function buildSoundLabel(param1:int, param2:String, param3:String) : String
      {
         ...
         var _loc10_:SoundAnimation = null;
         if(param3 != null)
         {
            _loc8_ = /^\s*(.*?)\s*$/g;
            param3 = param3.replace(_loc8_,"$1");
            if(param3.length == 0)
            {
               param3 = null;
            }
         ...
         var _loc5_:TiphonSprite = DofusEntities.getEntity(param1) as TiphonSprite;
         if(!_loc5_ || !_loc5_.look)
         {
            _log.error(param1 + " : donnés incomplètes pour ce bones, impossible de créer les sons");
            return null;
         }
         var _loc6_:int = int((TiphonUtility.getEntityWithoutMount(_loc5_) as TiphonSprite).look.getBone());
         var _loc7_:SoundBones = SoundBones.getSoundBonesById(_loc6_);
         if(_loc7_ != null)
         {
            _loc9_ = new Vector.<SoundEventParamWrapper>();
            for each(_loc10_ in _loc7_.getSoundAnimationByLabel(param2,param3))
            {
               _loc9_.push(new SoundEventParamWrapper(_loc10_.filename,_loc10_.volume,_loc10_.rolloff,_loc10_.automationDuration,_loc10_.automationVolume,_loc10_.automationFadeIn,_loc10_.automationFadeOut,_loc10_.noCutSilence));
            }
         }
