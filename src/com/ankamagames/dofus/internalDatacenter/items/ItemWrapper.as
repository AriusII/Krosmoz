package com.ankamagames.dofus.internalDatacenter.items
{
   ...
   public class ItemWrapper extends Item implements ISlotData, ICellZoneProvider, IDataCenter
   {
      ...
      {
         return !!hasOwnProperty("minRange") ? uint(this["minRange"]) : 0;
      }
      ...
      {
         return !!hasOwnProperty("range") ? uint(this["range"]) : 0;
      }
      ...
      public function get isCertificate() : Boolean
      {
         ...
         return Boolean(_loc1_) && _loc1_.typeId == ITEM_TYPE_CERTIFICATE;
      }
      
      public function get isEquipment() : Boolean
      {
         var _loc1_:Item = Item.getItemById(this.objectGID);
         return Boolean(_loc1_) && EQUIPMENT_SUPER_TYPES.indexOf(_loc1_.type.superTypeId) != -1;
      }
      
      public function get isUsable() : Boolean
      {
         var _loc1_:Item = Item.getItemById(this.objectGID);
         return Boolean(_loc1_) && (_loc1_.usable || _loc1_.targetable);
      }
      
      public function get belongsToSet() : Boolean
      {
         var _loc1_:Item = Item.getItemById(this.objectGID);
         return Boolean(_loc1_) && _loc1_.itemSetId != -1;
      }
      ...
      public function get favoriteEffect() : Vector.<EffectInstance>
      {
         ...
         if(PlayedCharacterManager.getInstance())
         {
            ...
            if(Boolean(_loc2_) && _loc3_.favoriteSubAreas.indexOf(_loc2_.id) != -1)
            {
               if(_loc3_.favoriteSubAreas && _loc3_.favoriteSubAreas.length && Boolean(_loc3_.favoriteSubAreasBonus))
               {
            ...
            switch(this.objectGID)
            {
               case OBJECT_GID_SOULSTONE:
                  ...
                  for each(_loc5_ in this.effects)
                  {
                     ...
                     if(_loc6_)
                     {
                        _loc7_ = _loc6_.grades[int(_loc5_.parameter0) - 1];
                        if((Boolean(_loc7_)) && _loc7_.level > _loc1_)
                        {
                           _loc1_ = int(_loc7_.level);
                           _loc2_ = _loc6_.name;
                        }
                     }
                  }
               ...
               case OBJECT_GID_SOULSTONE_MINIBOSS:
                  ...
                  {
                     _loc6_ = Monster.getMonsterById(int(_loc5_.parameter2));
                     if((Boolean(_loc6_)) && _loc6_.isMiniBoss)
                     {
               ...
               case OBJECT_GID_SOULSTONE_BOSS:
                  ...
                  {
                     _loc6_ = Monster.getMonsterById(int(_loc5_.parameter2));
                     if((Boolean(_loc6_)) && _loc6_.isBoss)
                     {
      ...
      public function getIconUri(param1:Boolean = true) : Uri
      {
         ...
         if(param1 && Boolean(this._uriPngMode))
         {
            return this._uriPngMode;
         }
         if(!param1 && Boolean(this._uri))
         {
            return this._uri;
         }
      ...
      private function updateEffects(param1:Vector.<ObjectEffect>) : void
      {
         ...
         var _loc3_:uint = 0;
         var _loc4_:uint = 0;
         if(Boolean(_loc2_) && _loc2_.isWeapon)
         {
         ...
         for each(_loc5_ in param1)
         {
            ...
            if(Boolean(_loc3_) && _loc6_.category == 2)
            {
