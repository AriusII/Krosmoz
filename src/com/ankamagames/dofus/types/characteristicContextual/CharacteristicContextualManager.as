package com.ankamagames.dofus.types.characteristicContextual
{
   ...
   public class CharacteristicContextualManager extends EventDispatcher
   {
      ...
      public function addStatContextual(param1:String, param2:IEntity, param3:TextFormat, param4:uint, param5:Number = 1, param6:uint = 2500) : CharacteristicContextual
      {
         ...
         var _loc8_:uint = uint(_loc7_.indexOf(Math.min(_loc7_[0],_loc7_[1],_loc7_[2],_loc7_[3])));
         switch(this._type)
         {
            case 1:
            ...
            if(_loc4_)
            {
               ...
               if(_loc4_._tweeningCurrentDistance < 1 / 8)
               {
                  ...
                  if(this._type == 2)
                  {
                     ...
                     if(!(_loc5_.referedEntity is DisplayObject) || Boolean(DisplayObject(_loc5_.referedEntity).parent))
                     {
                        _loc5_.x = (_loc7_.x + _loc7_.width / 2 - _loc5_.width / 2 - StageShareManager.stageOffsetX) / StageShareManager.stageScaleX;
                     }
                  }
               }
               else if(_loc4_._tweeningCurrentDistance < 1 / 4)
               {
                  _loc5_.alpha = _loc4_._tweeningCurrentDistance * 4;
                  if(this._type == 2)
                  {
               ...
               else if(_loc4_._tweeningCurrentDistance >= 3 / 4 && _loc4_._tweeningCurrentDistance < 1)
               {
                  _loc5_.alpha = 1 - _loc4_._tweeningCurrentDistance;
               }
               else if(_loc4_._tweeningCurrentDistance >= 1)
               {
...
class TweenData
{
   ...
   public function TweenData(param1:CharacteristicContextual, param2:IEntity, param3:Number, param4:uint)
   {
      super();
      this.context = param1;
      this.entity = param2;
