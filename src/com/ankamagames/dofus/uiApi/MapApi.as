package com.ankamagames.dofus.uiApi
{
   ...
   public class MapApi implements IApi
   {
      ...
      public function movePlayer(param1:int, param2:int, param3:int = -1) : void
      {
         ...
         if(_loc5_)
         {
            _loc7_ = param3 == -1 ? PlayedCharacterManager.getInstance().currentMap.worldId : uint(param3);
            _loc8_ = uint(PlayedCharacterManager.getInstance().currentSubArea.area.superArea.id);
            _loc9_ = uint(PlayedCharacterManager.getInstance().currentSubArea.area.id);
            _loc10_ = uint(PlayedCharacterManager.getInstance().currentSubArea.id);
            ...
            for each(_loc13_ in _loc5_)
            {
               ...
               if(_loc18_)
               {
                  _loc17_ = int(_loc18_.uiClass.currentWorldId);
               }
               ...
               if(Boolean(_loc14_.subArea) && _loc14_.subArea.id == _loc10_)
               {
                  _loc15_ += 100;
               }
