package com.ankamagames.dofus.uiApi
{
   ...
   public class PlayedCharacterApi implements IApi
   {
      ...
      public static function getTitle() : Title
      {
         ...
         var _loc1_:int = int((Kernel.getWorker().getFrame(TinselFrame) as TinselFrame).currentTitle);
         if(_loc1_)
         {
            return Title.getTitleById(_loc1_);
         }
         _loc3_ = getEntityInfos();
         if(Boolean(_loc3_) && Boolean(_loc3_.humanoidInfo))
         {
            for each(_loc4_ in _loc3_.humanoidInfo.options)
            {
               if(_loc4_ is HumanOptionTitle)
               {
                  _loc1_ = int(_loc4_.titleId);
               }
            }
            return Title.getTitleById(_loc1_);
         }
      ...
      public static function getOrnament() : Ornament
      {
         ...
         var _loc1_:int = int((Kernel.getWorker().getFrame(TinselFrame) as TinselFrame).currentOrnament);
         if(_loc1_)
         {
            return Ornament.getOrnamentById(_loc1_);
         }
         return null;
      }
      ...
      public static function getEntityInfos() : GameRolePlayCharacterInformations
      {
         ...
         return _loc1_.getEntityInfos(PlayedCharacterManager.getInstance().infos.id) as GameRolePlayCharacterInformations;
      }
      ...
      public static function isCreature() : Boolean
      {
         ...
         var _loc1_:uint = getBone();
         for each(_loc2_ in Breed.getBreeds())
         {
            if(_loc2_.creatureBonesId == _loc1_)
            {
               return true;
            }
         }
