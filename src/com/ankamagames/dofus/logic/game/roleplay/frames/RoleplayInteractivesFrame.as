package com.ankamagames.dofus.logic.game.roleplay.frames
{
   ...
   public class RoleplayInteractivesFrame implements Frame
   {
      ...
      public static function getCursor(param1:int, param2:Boolean = true, param3:Boolean = true) : Sprite
      {
         ...
         if(Boolean(cursorList[param1]) && param3)
         {
      ...
      private function updateStatedElement(param1:StatedElement) : void
      {
         ...
         if(param1.elementState != COLLECTABLE_COLLECTING_STATE_ID)
         {
            this._statedElementsTargetAnimation[_loc3_] = {
               "elemId":param1.elementId,
               "animation":"AnimState" + param1.elementState
            };
            _loc3_.addEventListener(TiphonEvent.RENDER_SUCCEED,this.onAnimRendered);
         }
         _loc3_.setAnimationAndDirection("AnimState" + param1.elementState,0);
         if(param1.elementState == COLLECTABLE_COLLECTING_STATE_ID && this._ie[_loc2_] && this._ie[_loc2_].element.elementId == param1.elementId)
         {
            _loc4_ = Interactive.getInteractiveById(this._ie[_loc2_].element.elementTypeId);
            if(!_loc4_)
            {
               return;
            }
            if(_loc4_.actionId == COLLECTABLE_INTERACTIVE_ACTION_ID)
            {
               this._collectableSpritesToBeStopped[_loc3_] = null;
            }
         }
         else
         {
            delete this._collectableSpritesToBeStopped[_loc3_];
         }
      }
      ...
      private function click(param1:MouseEvent) : void
      {
         ...
         for each(_loc10_ in _loc2_.element.disabledSkills)
         {
            ...
            if(_loc8_.parentJobId != 1)
            {
               ...
               {
                  _loc16_ = int(_loc15_.jobExperience.jobLevel);
         ...
         for each(_loc13_ in _loc4_)
         {
            if(_loc13_.enabled)
            {
               _loc12_ = int(_loc4_.indexOf(_loc13_));
               _loc11_++;
            }
         }
         if(_loc11_ == 1)
      ...
      private function showInteractiveElementNotification(param1:Array) : void
      {
         ...
         if(param1.length > 0)
         {
            ...
            if(_loc6_.length > 0)
            {
               ...
               if(_loc8_ != "")
               {
                  _loc2_ += I18n.getUiText("ui.skill.levelLow",[(_loc6_.length > 1 ? "<ul>" : "") + _loc8_ + (_loc6_.length > 1 ? "</ul>" : ".")]);
               }
               if(_loc7_ != "")
               {
                  _loc2_ += I18n.getUiText("ui.skill.toolNeeded",[(_loc6_.length > 1 ? "<ul>" : "") + _loc7_ + (_loc6_.length > 1 ? "</ul>" : ".")]);
               }
            }
            else
            {
               ...
               {
                  _loc12_ += (param1.length > 1 ? "<li>" : "") + _loc4_.value[0] + (param1.length > 1 ? "</li>" : "");
               }
               _loc2_ = I18n.getUiText("ui.skill.jobNotKnown",[(param1.length > 1 ? "<ul>" : "") + _loc12_ + (param1.length > 1 ? "</ul>" : ".")]);
               _loc2_ += "\n";
               _loc2_ += I18n.getUiText("ui.npc.learnJobs",[_loc9_.name,_loc10_.x,_loc10_.y]);
               _loc5_ = _loc9_.name;
               _loc3_ = true;
            }
            if(_loc2_ != "")
