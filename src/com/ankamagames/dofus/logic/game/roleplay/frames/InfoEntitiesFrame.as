package com.ankamagames.dofus.logic.game.roleplay.frames
{
   ...
   public class InfoEntitiesFrame implements Frame
   {
      ...
      private function entityMovementCompleteHandler(param1:IEntity) : void
      {
         ...
         var _loc3_:int = int(this._playersNames.indexOf(_loc2_));
         if(_loc3_ != -1)
         {
            _loc4_ = int(this._movableEntities.indexOf(_loc3_));
      ...
      private function gameRolePlayShowActorHandler(param1:Object) : void
      {
         ...
         else
         {
            ...
            if(_loc4_ == null)
            {
               this.addEntity(_loc3_,_loc2_.name);
            }
            else
            {
      ...
      private function removeElementHandler(param1:int) : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc3_ = int(this._playersNames.indexOf(_loc2_));
            if(_loc3_ != -1)
            {
               _loc4_ = int(this._movableEntities.indexOf(_loc3_));
               if(_loc4_ != -1)
      ...
      private function updateEntities() : void
      {
         ...
         for each(_loc1_ in _loc3_)
         {
            ...
            if(_loc2_ != null)
            {
               this.addEntity(_loc1_,_loc2_.name);
            }
            else
            {
      ...
      {
         var _loc2_:int = 0;
         var _loc3_:int = int(this._playersNames.length);
         _loc2_ = 0;
         while(_loc2_ < _loc3_)
         {
      ...
      private function addEntity(param1:int, param2:String) : void
      {
         ...
         var _loc5_:DisplayedEntity = null;
         var _loc6_:IEntity = null;
         var _loc7_:int = 0;
         if(this.getEntity(param1) == null)
         {
            _loc3_ = new Label();
            _loc3_.css = new Uri(XmlConfig.getInstance().getEntry("config.ui.skin") + "css/normal.css");
            _loc3_.text = param2;
            _loc3_.mouseEnabled = true;
            _loc3_.bgColor = XmlConfig.getInstance().getEntry("colors.tooltip.bg");
            _loc3_.bgAlpha = XmlConfig.getInstance().getEntry("colors.tooltip.bg.alpha");
            _loc3_.width = _loc3_.textWidth + 7;
            _loc3_.height += 4;
            _loc3_.buttonMode = true;
            _loc3_.addEventListener(MouseEvent.CLICK,this.onTooltipClicked);
            if(param1 == PlayedCharacterApi.id())
            {
               _loc3_.colorText = XmlConfig.getInstance().getEntry("colors.tooltip.text.red");
            }
            _loc4_ = DofusEntities.getEntity(param1) as TiphonSprite;
            if(_loc4_ == null)
            {
               _loc5_ = new DisplayedEntity(param1,_loc3_);
            }
            else
            {
               _loc5_ = new DisplayedEntity(param1,_loc3_,this.getBounds(param1));
               if(StealthBones.getStealthBonesById(_loc4_.look.getBone()))
               {
                  return;
               }
               this._labelContainer.addChild(_loc3_);
            }
            this.updateDisplayedEntityPosition(_loc5_);
            this._playersNames.push(_loc5_);
            _loc6_ = DofusEntities.getEntity(param1);
            if(_loc6_ is IMovable)
            {
               if(IMovable(_loc6_).isMoving)
               {
                  this._movableEntities.push(this._playersNames.indexOf(_loc5_));
               }
               else
               {
                  _loc7_ = int(this._movableEntities.indexOf(this._playersNames.indexOf(_loc5_)));
                  if(_loc7_ != -1)
                  {
                     this._movableEntities.splice(_loc7_,1);
                  }
               }
            }
         }
      ...
      private function updateTextsPosition(param1:Event) : void
      {
         ...
         {
            _loc3_ = int(this._movableEntities.length);
            _loc2_ = 0;
            while(_loc2_ < _loc3_)
            {
      ...
      private function waitForEntity(param1:Event) : void
      {
         ...
         for each(_loc2_ in this._waitList)
         {
            ...
            if(_loc3_ != null && DofusEntities.getEntity(_loc2_) != null)
            {
               ...
               if(_loc3_.target.width != 0 && _loc3_.target.height != 0)
               {
      ...
      private function getBounds(param1:int) : IRectangle
      {
         ...
         if(_loc3_)
         {
            ...
            _loc4_ = _loc6_ = new Rectangle2(_loc5_.x,_loc5_.y,_loc5_.width,_loc5_.height);
            if(_loc4_.y <= _loc4_.height)
            {
               ...
               if(_loc7_)
               {
                  ...
                  _loc4_ = _loc6_ = new Rectangle2(_loc5_.x,_loc5_.y + _loc4_.height + 30,_loc5_.width,_loc5_.height);
               }
            }
         }
         if(!_loc4_)
...
class DisplayedEntity
{
   ...
   public function DisplayedEntity(param1:int = 0, param2:Label = null, param3:IRectangle = null)
   {
      super();
