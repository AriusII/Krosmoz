package com.ankamagames.dofus.logic.game.common.managers
{
   ...
   public class PlayedCharacterManager implements IDestroyable
   {
      ...
      public function get isIncarnation() : Boolean
      {
         ...
         var _loc1_:Object = InventoryManager.getInstance().inventory.getView("equipment");
         for each(_loc2_ in _loc1_.content)
         {
            if(_loc2_)
            {
               for each(_loc3_ in _loc2_.effectsList)
               {
                  if(_loc3_.actionId == 669)
                  {
                     return true;
                  }
               }
      ...
      {
         var _loc3_:int = 0;
         var _loc2_:int = int(this.infos.level);
         var _loc4_:int = 1;
         if(param1 < _loc2_)
         {
            _loc4_ = -1;
