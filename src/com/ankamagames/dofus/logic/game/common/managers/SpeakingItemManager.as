package com.ankamagames.dofus.logic.game.common.managers
{
   ...
   public class SpeakingItemManager implements IDestroyable
   {
      ...
      public function triggerEvent(param1:int) : void
      {
         ...
         var _loc2_:Boolean = Boolean(OptionManager.getOptionManager("chat").letLivingObjectTalk);
         if(!_loc2_)
         {
            return;
         }
         ...
         this._nextMessageCount -= (_loc3_.length - 1) / 4;
         ...
         if(_loc5_)
         {
            ...
            while(_loc9_ < 10)
            {
               _loc10_ = Number(_loc7_[Math.floor(Math.random() * _loc7_.length)]);
