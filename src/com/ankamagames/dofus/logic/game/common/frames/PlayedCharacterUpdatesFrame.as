package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class PlayedCharacterUpdatesFrame implements Frame
   {
         ...
         switch(true)
         {
            ...
            case msg is MapComplementaryInformationsDataMessage:
               ...
               for each(grai in mcidmsg.actors)
               {
                  ...
                  if(Boolean(grpci) && grpci.contextualId == PlayedCharacterManager.getInstance().id)
                  {
            ...
            case msg is SpellUpgradeSuccessMessage:
               ...
               for each(sw in PlayedCharacterManager.getInstance().spellsInventory)
               {
                  if(sw.id == susmsg.spellId)
                  {
                     ...
                     if(oldSw)
                     {
                        previousCooldown = int(oldSw.actualCooldown);
                     }
            ...
            case msg is CharacterLevelUpMessage:
               ...
               switch(messageId)
               {
                  case CharacterLevelUpMessage.protocolId:
                     spellPointEarned = uint(clumsg.newLevel - PlayedCharacterManager.getInstance().infos.level);
                     caracPointEarned = uint((clumsg.newLevel - PlayedCharacterManager.getInstance().infos.level) * 5);
                     healPointEarned = uint((clumsg.newLevel - PlayedCharacterManager.getInstance().infos.level) * 5);
                     newSpell = new Array();
                     ...
                     else
                     {
                        ...
                        if(newSpell.length > 0)
                        {
                           ...
                           levelSpellObtention = int(selectedSpell.minPlayerLevel);
                        }
                     }
            ...
            case msg is CompassResetMessage:
               ...
               switch(crmsg.type)
               {
                  ...
                  case CompassTypeEnum.COMPASS_TYPE_PARTY:
                     ...
                     name += "_" + PlayedCharacterManager.getInstance().followingPlayerId;
                     break;
               }
            ...
            case msg is CompassUpdateMessage:
               ...
               {
                  case CompassTypeEnum.COMPASS_TYPE_PARTY:
                     memberId = int(CompassUpdatePartyMemberMessage(msg).memberId);
               ...
               KernelEventsManager.getInstance().processCallback(HookList.AddMapFlag,name,legend,PlayedCharacterManager.getInstance().currentWorldMap.id,cumsg.worldX,cumsg.worldY,color,false);
               return true;
            default:
               return false;
         }
      ...
      public function updateCharacterStatsList(param1:CharacterStatsListMessage) : void
      {
         ...
         if(Boolean(_loc2_) && _loc2_.energyPoints > param1.stats.energyPoints)
         {
      ...
      public function getPlayerSet(param1:uint) : PlayerSetInfo
      {
         ...
         var _loc6_:int = 0;
         var _loc7_:int = 0;
         var _loc2_:int = int(this.setList.length);
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_)
         {
