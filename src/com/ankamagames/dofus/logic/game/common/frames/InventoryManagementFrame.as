package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class InventoryManagementFrame implements Frame
   {
      ...
      public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case msg is ObjectQuantityMessage:
               ...
               {
                  if(Boolean(shortcutQty) && shortcutQty.id == oqm.objectUID)
                  {
            ...
            case msg is ObjectsQuantityMessage:
               ...
               for each(objoqm in osqm.objectsUIDAndQty)
               {
                  ...
                  {
                     if(Boolean(shortcutsQty) && shortcutsQty.id == objoqm.objectUID)
                     {
            ...
            case msg is PresetSetPositionAction:
               ...
               for each(realitem in InventoryManager.getInstance().inventory.getView("real").content)
               {
                  if(Item.getItemById(realitem.objectGID).typeId == Inventory.HIDDEN_TYPE_ID)
                  {
                     ...
                     if(Boolean(realitem.effects) && Boolean(realitem.effects.length))
                     {
      ...
      private function getShortcutWrapperPropFromShortcut(param1:Object) : Object
      {
         ...
         {
            _loc2_ = uint((param1 as ShortcutObjectItem).itemUID);
            _loc3_ = uint((param1 as ShortcutObjectItem).itemGID);
            _loc4_ = 0;
         }
