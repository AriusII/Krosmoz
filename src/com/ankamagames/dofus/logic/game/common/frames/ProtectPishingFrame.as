package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class ProtectPishingFrame implements Frame
   {
      ...
      private var _advancedInputBufferRef:Dictionary = new Dictionary(true);
      ...
      {
         if(Boolean(_passwordHash) && Boolean(_passwordLength))
         {
      ...
      public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            case param1 is ChangeMessage:
               ...
               if(Boolean(_loc2_) && Boolean(this._cancelTarget[_loc2_.textfield]))
               {
      ...
      {
         var _loc4_:uint = 0;
         this._globalBuffer += param1.text;
         if(!(param1.target is TextField && TextField(param1.target).parent is Input && Boolean(Input(TextField(param1.target).parent).getUi()) && !Input(TextField(param1.target).parent).getUi().uiModule.trusted))
         {
            return;
         }
         this._globalModBuffer += param1.text;
         ...
         {
            _loc4_ = uint(_loc2_.length - _passwordLength + 1);
         ...
         {
            _loc4_ = uint(this._globalBuffer.length - _passwordLength + 1);
         ...
         {
            _loc4_ = uint(this._globalModBuffer.length - _passwordLength + 1);
      ...
      {
         var _loc4_:uint = uint(param1.length - param3 + 1);
         var _loc5_:uint = 0;
         while(_loc5_ < _loc4_)
         {
      ...
      protected function onChange(param1:Event) : void
      {
         ...
         if(_loc4_.length >= _passwordLength)
         {
            _loc5_ = uint(_loc4_.length - _passwordLength + 1);
