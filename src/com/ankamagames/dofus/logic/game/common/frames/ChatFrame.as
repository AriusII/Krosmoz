package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class ChatFrame implements Frame
   {
      ...
      private var _cssUri:String = XmlConfig.getInstance().getEntry("config.ui.skin") + "css/chat.css";
      ...
      public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case msg is ChatTextOutputAction:
               ...
               while(true)
               {
                  leftIndex = int(content.indexOf("["));
                  if(leftIndex == -1)
                  {
                     break;
                  }
                  rightIndex = int(content.indexOf("]"));
                  if(rightIndex == -1)
                  {
                     break;
                  }
            ...
            case msg is ChatServerWithObjectMessage:
               ...
               numItem = int(scwomsg.objects.length);
            ...
            case msg is ChatServerMessage:
               ...
               if(csmsg.channel == ChatActivableChannelsEnum.CHANNEL_GLOBAL)
               {
                  ...
                  if(speakerEntity is TiphonSprite)
                  {
                     ...
                     isCreatureMode = Boolean(Kernel.getWorker().getFrame(RoleplayEntitiesFrame)) && RoleplayEntitiesFrame(Kernel.getWorker().getFrame(RoleplayEntitiesFrame)).isCreatureMode;
                     if(Boolean(rider) && !isCreatureMode)
                     {
                        tooltipTarget = rider;
                     }
            ...
            case msg is ChatServerCopyWithObjectMessage:
               ...
               numItemc = int(cscwomsg.objects.length);
            ...
            case msg is BasicWhoIsMessage:
               ...
               if(bwimsg.socialGroups.length > 0)
               {
                  for each(socGroup in bwimsg.socialGroups)
                  {
                     ...
                     {
                        text += " " + I18n.getUiText("ui.common.guild") + " {guild," + (socGroup as BasicGuildInformations).guildId + "::" + (socGroup as BasicGuildInformations).guildName + "}";
                     }
                     else if(socGroup is BasicAllianceInformations)
                     {
                        text += " " + I18n.getUiText("ui.common.alliance") + " {alliance," + (socGroup as BasicAllianceInformations).allianceId + "::[" + (socGroup as BasicAllianceInformations).allianceTag + "]}";
                     }
                  }
               }
               if(bwimsg.playerState == PlayerStateEnum.NOT_CONNECTED)
               {
                  text += " (" + I18n.getUiText("ui.common.disconnected") + ")";
               }
            ...
            case msg is ExchangeGuildTaxCollectorGetMessage:
               ...
               for each(object in egtcgmsg.objectsInfos)
               {
                  ...
                  textObjects += object.quantity + "x" + Item.getItemById(object.objectUID).name;
               }
               if(textObjects != "")
               {
      ...
      public function checkCensored(param1:String, param2:uint, param3:uint, param4:String) : Array
      {
         ...
         if(OptionManager.getOptionManager("chat").filterInsult && param2 != 8 && param2 != 10 && param2 != 11 && param2 != 666)
         {
               ...
               for each(_loc20_ in _loc19_)
               {
                  ...
                  if(this._aCensoredWords)
                  {
                     if(this._aCensoredWords[_loc21_])
                     {
                        ...
                        {
                           _loc22_ += _loc17_[_loc21_.charCodeAt(_loc23_) % 5];
                           _loc23_++;
                        }
                     }
                     else
                        ...
                        for(_loc24_ in this._aCensoredWords)
                        {
                           if(this._aCensoredWords[_loc24_] == 2)
                           {
                              if(_loc21_.indexOf(_loc24_) != -1)
                              {
                                 ...
                                 {
                                    _loc22_ += _loc17_[_loc21_.charCodeAt(_loc25_) % 5];
                                    _loc25_++;
                                 }
                              }
                           }
            ...
            else
            {
               ...
               for(_loc28_ in this._aCensoredWords)
               {
                  ...
                  while(_loc29_ != -1)
                  {
                     _loc29_ = int(_loc27_.indexOf(_loc28_));
                     if(_loc29_ != -1)
                     {
         ...
         for each(_loc12_ in _loc6_)
         {
            ...
            if(_loc31_.formate)
            {
               _loc30_ += HtmlManager.addLink("[" + _loc31_.url + "]","event:chatLinkRelease," + _loc31_.url + "," + param3 + "," + param4,{"bold":true});
               _loc8_ = true;
            }
            if(_loc30_ == "")
            {
