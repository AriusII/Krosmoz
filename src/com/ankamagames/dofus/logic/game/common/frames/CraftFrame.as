package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class CraftFrame implements Frame
   {
      ...
      public function processExchangeOkMultiCraftMessage(param1:ExchangeOkMultiCraftMessage) : void
      {
         ...
         var _loc3_:String = "";
         var _loc4_:uint = _loc2_.initiatorId;
         if(_loc2_.initiatorId == PlayedCharacterManager.getInstance().infos.id)
         {
            if(_loc2_.initiatorId == this.crafterInfos.id)
            {
               this._isCrafter = true;
               _loc3_ = this.customerInfos.name;
            }
            else
            {
               this._isCrafter = false;
               _loc3_ = this.crafterInfos.name;
            }
         }
         ...
         if(!this.socialFrame.isIgnored(_loc3_))
         {
            KernelEventsManager.getInstance().processCallback(CraftHookList.ExchangeMultiCraftRequest,_loc2_.role,_loc3_,_loc4_);
         }
      }
      ...
      public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case param1 is ExchangeItemGoldAddAsPaymentAction:
               ...
               {
                  _loc5_ = uint(PaymentTypeEnum.PAYMENT_ON_SUCCESS_ONLY);
               }
               else
               {
                  _loc5_ = uint(PaymentTypeEnum.PAYMENT_IN_ANY_CASE);
               }
            ...
            case param1 is ExchangeItemObjectAddAsPaymentAction:
               ...
               {
                  _loc8_ = uint(PaymentTypeEnum.PAYMENT_ON_SUCCESS_ONLY);
                  _loc9_ = this.paymentCraftList.objectsPaymentOnlySuccess;
               }
               else
               {
                  _loc8_ = uint(PaymentTypeEnum.PAYMENT_IN_ANY_CASE);
                  _loc9_ = this.paymentCraftList.objectsPaymentOnlySuccess;
               }
            ...
            case param1 is ExchangeCraftResultMessage:
               ...
               switch(_loc17_)
               {
                  ...
                  case ExchangeCraftResultMagicWithObjectDescMessage.protocolId:
                           ...
                           if(_loc69_ is ObjectEffectInteger || _loc69_ is ObjectEffectDice)
                           {
                              ...
                              for each(_loc71_ in _loc63_)
                              {
                                 if((_loc71_ is ObjectEffectInteger || _loc71_ is ObjectEffectDice) && _loc71_.actionId == _loc69_.actionId)
                                 {
                                    ...
                                    {
                                       _loc72_ *= ObjectEffectInteger(_loc69_).value;
                                       _loc73_ *= ObjectEffectInteger(_loc71_).value;
                                       if(_loc73_ != _loc72_)
                                       {
                                          ...
                                          _loc61_ += " " + _loc75_.description + ",";
                                          _loc61_ = _loc61_.replace("+-","-");
                                          _loc61_ = _loc61_.replace("--","+");
                                          _loc64_ = _loc75_;
                                       }
                                    }
                                    else if(_loc71_ is ObjectEffectDice)
                                    {
                                       _loc72_ = int(ObjectEffectDice(_loc69_).diceNum);
                                       _loc73_ = int(ObjectEffectDice(_loc71_).diceNum);
                                       if(_loc73_ != _loc72_)
                                       {
                                          _loc74_ = _loc73_ - _loc72_;
                     ...
                     if(_loc62_)
                     {
                        _loc67_ += I18n.getUiText("ui.craft.success") + I18n.getUiText("ui.common.colon");
                     }
                     else
                     {
                        _loc67_ += I18n.getUiText("ui.craft.failure") + I18n.getUiText("ui.common.colon");
                     }
                     _loc67_ += _loc61_;
                     if(_loc66_ != "")
                     {
                     ...
                     if(_loc61_ == "" && _loc66_ == "")
                     {
                        _loc67_ = _loc67_.substring(0,_loc67_.length - 2);
                     }
...
class PlayerExchangeCraftList
{
   ...
   public function PlayerExchangeCraftList()
   {
      super();
