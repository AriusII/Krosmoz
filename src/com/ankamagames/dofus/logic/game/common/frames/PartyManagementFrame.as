      ...
      public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case msg is PartyInvitationDungeonMessage:
               ...
               NotificationManager.getInstance().addButtonToNotification(pidmsgNid,I18n.getUiText("ui.common.accept"),"openSocial",[pidmsg.partyId],true,130);
            ...
            case msg is PartyJoinMessage:
               ...
               else
               {
                  ...
                  for each(memberJoin in pjmsg.members)
                  {
                     partyMember = new PartyMemberWrapper(memberJoin.id,memberJoin.name,memberJoin.status.statusId,true,false,memberJoin.level,memberJoin.entityLook,memberJoin.lifePoints,memberJoin.maxLifePoints,memberJoin.initiative,memberJoin.prospecting,memberJoin.alignmentSide,memberJoin.regenRate);
                     if(memberJoin.id == pjmsg.partyLeaderId)
                     {
                        partyMember.isLeader = true;
                     }
            ...
            case msg is TeleportBuddiesRequestedMessage:
               ...
               if(poorBuddiesNames != "")
               {
                  ...
                  prinText += " " + PatternDecoder.combine(I18n.getUiText("ui.party.teleportCriterionFallenAngels",[poorBuddiesNames]),"n",poorBuddiesNames.split(", ").length == 1);
               }
            ...
            case msg is DungeonPartyFinderRoomContentUpdateMessage:
               ...
               iFD = int(tempDjFighters.length - 1);
               while(iFD >= 0)
               {
            ...
            case msg is FightEndingMessage:
               ...
               if(this._lastFightType == FightTypeEnum.FIGHT_TYPE_PVP_ARENA)
               {
                  ...
                  this._arenaReadyPartyMemberIds = new Array();
                  KernelEventsManager.getInstance().processCallback(RoleplayHookList.ArenaRegistrationStatusUpdate,this._isArenaRegistered,this._arenaCurrentStatus);
                  if(Boolean(this._arenaLeader) && PlayedCharacterManager.getInstance().id == this._arenaLeader.id)
                  {
            ...
            case msg is GameRolePlayRemoveChallengeMessage:
               ...
               fightNotificationIndex = int(this._partyFightNotification.indexOf("partyFight" + grpcm.fightId));
      ...
      private function onTimerTick(param1:TimerEvent) : void
      {
         ...
         for each(_loc2_ in this._partyMembers)
         {
            if(_loc2_.lifePoints < _loc2_.maxLifePoints && _loc2_.regenRate > 0)
            {
               ...
               {
                  _loc5_ = uint(_loc2_.maxLifePoints);
               }
         ...
         for each(_loc2_ in this._arenaPartyMembers)
         {
            if(_loc2_.lifePoints < _loc2_.maxLifePoints && _loc2_.regenRate > 0)
            {
               ...
               {
                  _loc9_ = uint(_loc2_.maxLifePoints);
               }
...
class LifePointTickManager
{
   ...
   public function LifePointTickManager()
   {
      super();
   }
}
