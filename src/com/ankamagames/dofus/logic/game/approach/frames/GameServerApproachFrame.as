package com.ankamagames.dofus.logic.game.approach.frames
{
   ...
   public class GameServerApproachFrame implements Frame
   {
      ...
      private var _charactersToRecolorList:Array = new Array();
      
      private var _charactersToRenameList:Array = new Array();
      
      private var _charactersToRelookList:Array = new Array();
         ...
         switch(true)
         {
            ...
            case msg is CharactersListMessage:
               ...
               if(this._charactersList.length)
               {
                  ...
                  else if(Dofus.getInstance().options && Dofus.getInstance().options.autoConnectType == 2 && QueueFrame.getInstance().allowAutoConnectCharacter)
                  {
                     if((server.gameTypeId != 1 || this._charactersList[0].deathState == 0) && (!SecureModeManager.getInstance().active || !this.isCharacterWaitingForChange(this._charactersList[0].id)))
                     {
                        openCharsList = false;
                        ...
                        fakacsa.characterId = this._charactersList[0].id;
                        this.process(fakacsa);
                        QueueFrame.getInstance().allowAutoConnectCharacter = false;
                     }
                  }
                  if(openCharsList)
                  {
            ...
            case msg is CharacterDeselectionAction:
               ...
               _log.debug("CharacterDeselectionAction  _dontAutoConnectCharacter " + false);
               return true;
            ...
            case msg is CharacterReplayRequestAction:
               ...
               {
                  characterId = int((msg as CharacterReplayRequestAction).characterId);
                  bTutorial = false;
                  isReplay = true;
               }
