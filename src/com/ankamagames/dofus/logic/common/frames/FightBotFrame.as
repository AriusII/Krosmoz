   ...
   public class FightBotFrame implements Frame
   {
      ...
      private function randomOver(... rest) : void
      {
         ...
         for each(_loc7_ in Berilia.getInstance().uiList)
         {
            ...
            {
               if(Boolean(_loc11_.mouseChildren) || _loc11_.mouseEnabled)
               {
                  _loc6_.push(_loc11_);
               }
            }
      ...
      private function castSpell(param1:uint, param2:Boolean) : void
      {
         ...
         if(param2)
         {
            _loc4_ = uint(FightEntitiesFrame.getCurrentInstance().getEntityInfos(PlayedCharacterManager.getInstance().id).disposition.cellId);
         }
         else
         {
            _loc5_ = [];
            ...
            _loc4_ = uint(_loc5_[Math.floor(_loc5_.length * Math.random())]);
         }
