package com.ankamagames.berilia.managers
{
   ...
   public class BindsManager extends GenericEventsManager
   {
      ...
      private var _avaibleKeyboard:Array = new Array();
      ...
      override public function initialize() : void
      {
         ...
         for each(_loc5_ in _loc3_)
         {
            ...
            {
               _loc4_.push(new Uri(_loc5_.url));
            }
         }
      ...
      public function getBind(param1:Bind, param2:Boolean = false) : Bind
      {
         ...
         var _loc3_:int = -1;
         var _loc4_:int = int(this._aRegisterKey.length);
         while(++_loc3_ < _loc4_)
         {
      ...
      public function isRegister(param1:Bind) : Boolean
      {
         ...
         {
            if(Boolean(this._aRegisterKey[_loc2_]) && Boolean(this._aRegisterKey[_loc2_].equals(param1)))
            {
               return true;
            }
            _loc2_++;
      ...
      public function isPermanent(param1:Bind) : Boolean
      {
         ...
         {
            if(Boolean(this._aRegisterKey[_loc3_]) && Boolean(this._aRegisterKey[_loc3_].equals(param1)))
            {
      ...
      public function isRegisteredName(param1:String) : Boolean
      {
         ...
         while(_loc2_ < this._aRegisterKey.length)
         {
            if(Boolean(this._aRegisterKey[_loc2_]) && this._aRegisterKey[_loc2_].targetedShortcut == param1)
            {
               return true;
            }
            _loc2_++;
      ...
      public function addBind(param1:Bind) : void
      {
         ...
         while(_loc2_ < this._aRegisterKey.length)
         {
            if(Boolean(this._aRegisterKey[_loc2_]) && Bind(this._aRegisterKey[_loc2_]).targetedShortcut == param1.targetedShortcut)
            {
      ...
      {
         return _aEvent[param1.targetedShortcut] != null || !param2 && Boolean(_aEvent["ALL"]);
      }
      ...
      public function processCallback(param1:*, ... rest) : void
      {
         ...
         if(_aEvent[_loc3_.targetedShortcut])
         {
            ...
            for each(_loc4_ in _loc6_)
            {
               ...
               if(_loc4_)
               {
                  if(Boolean(Berilia.getInstance().getUi(_loc4_.listener)) && Berilia.getInstance().getUi(_loc4_.listener).depth < Berilia.getInstance().highestModalDepth)
                  {
                     return;
                  }
                  _log.debug("Dispatch " + rest + " to " + _loc4_.listener);
      ...
      public function newShortcut(param1:Shortcut) : void
      {
         ...
         while(_loc3_ < this._waitingBinds.length)
         {
            ...
            if(_loc2_.targetedShortcut == param1.name)
            {
               break;
            }
            _loc3_++;
         ...
         {
            _log.error(_loc2_.toString() + " cannot be bind.");
            return;
         }
      ...
      public function checkBinds() : void
      {
         ...
         if(Boolean(this._bindsToCheck) && this._bindsToCheck.length > 0)
         {
            _loc1_ = false;
      ...
      private function fillShortcutsEnum() : void
      {
         ...
         for(_loc1_ in ShortcutsEnum.BASIC_SHORTCUT_KEYCODE)
         {
            ShortcutsEnum.BASIC_SHORTCUT_NAME[ShortcutsEnum.BASIC_SHORTCUT_KEYCODE] = _loc1_;
         }
      }
      ...
      private function parseBindsXml(param1:String, param2:Boolean) : void
      {
         ...
         {
            _loc14_ = 0;
            while(_loc14_ < this._aRegisterKey.length)
            {
               if(!this._aRegisterKey[_loc14_] || this._aRegisterKey[_loc14_].targetedShortcut == "ALL" && _loc14_ != this._aRegisterKey.length - 1)
               {
                  if(!_loc10_)
                  {
            ...
            _loc15_ = !!_loc10_ ? int(_loc10_.length) : 0;
            _loc14_ = 0;
            while(_loc14_ < _loc15_)
            {
               this._aRegisterKey.splice(_loc10_[_loc14_],1);
               _loc14_++;
            }
            _loc14_ = 0;
            while(_loc14_ < this._aRegisterKey.length)
            {
               if(this._aRegisterKey[_loc14_])
               {
                  _loc9_[this._aRegisterKey[_loc14_].targetedShortcut] = {
                     "exist":(!!Shortcut.getShortcutByName(this._aRegisterKey[_loc14_].targetedShortcut) ? true : false),
                     "bind":this._aRegisterKey[_loc14_],
                     "index":_loc14_
                  };
               }
               _loc14_++;
         ...
         for each(_loc11_ in _loc3_..bind)
         {
            ...
            {
               _log.error(_loc4_.toString() + " cannot be bind.");
            }
            else
            {
         ...
         _loc12_ = Shortcut.getShortcuts().length > 0;
         if(!_loc12_)
         {
         ...
         {
            if(!_loc13_.exist || _loc12_ && !Shortcut.getShortcutByName(_loc13_.bind.targetedShortcut))
            {
               if(!_loc12_)
               {
                  this._bindsToCheck.push(_loc13_);
               }
               else
               {
                  this._aRegisterKey.splice(_loc13_.index,1,null);
               }
            }
         }
         if(_loc12_)
