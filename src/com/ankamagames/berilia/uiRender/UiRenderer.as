package com.ankamagames.berilia.uiRender
{
   ...
   public class UiRenderer extends EventDispatcher
   {
      ...
      public function makeChilds(param1:Array, param2:GraphicContainer, param3:Boolean = false) : void
      {
         ...
         var aChildLength:int = int(aChild.length);
         i = 0;
         for(; i < aChildLength; i++)
         {
            try
            {
               be = aChild[i];
            ...
            else if(Boolean(this._uiDef) && this._uiDef.debug)
            {
      ...
      {
         var modalContainer:ContainerElement;
         var size:GraphicSize;
         ...
         modalContainer = new ContainerElement();
         size = new GraphicSize();
