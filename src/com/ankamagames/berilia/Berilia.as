package com.ankamagames.berilia
{
   ...
   public class Berilia extends EventDispatcher
   {
      ...
      public function addUIListener(param1:IInterfaceListener) : void
      {
         ...
         var _loc2_:int = int(this._UISoundListeners.indexOf(param1));
         if(_loc2_ == -1)
         {
      ...
      {
         var obj:DynamicSecureObject;
         var doIt:DisplayObject;
         var stopTimer:int;
         ...
         doIt = StageShareManager.stage.focus;
         while(doIt)
         {
         ...
         if(UiRootContainer(ui).uiClass)
         {
            ...
            for each(varName in variables)
            {
               ...
               {
                  if(getQualifiedClassName(UiRootContainer(ui).uiClass[varName]).indexOf("Api") && UiRootContainer(ui).uiClass[varName] is Object && Boolean(Object(UiRootContainer(ui).uiClass[varName]).hasOwnProperty("destroy")))
                  {
         ...
         if(Boolean(linkCursor) && Boolean(linkCursor.data.hasOwnProperty("currentHolder")))
         {
      ...
      public function unloadUiEvents(param1:String, param2:Boolean = false) : void
      {
         ...
         {
            if((_loc5_ != null || _loc5_ != "null") && UIEventManager.getInstance().instances[_loc5_] && Boolean(UIEventManager.getInstance().instances[_loc5_].instance) && Boolean(UIEventManager.getInstance().instances[_loc5_].instance.topParent) && UIEventManager.getInstance().instances[_loc5_].instance.topParent.name == param1)
            {
