package com.ankamagames.berilia.components
{
   ...
   public class Slot extends ButtonContainer implements ISlotDataHolder, FinalizableUIComponent, IDragAndDropHandler
   {
      ...
      public function refresh() : void
      {
         ...
         else if(Boolean(this._data) && Boolean(this._data.timer))
         {
         ...
         this._icon.cacheAsBitmap = Boolean(this._icon.uri) && this._icon.uri.tag == NEED_CACHE_AS_BITMAP;
         if(this._backgroundIcon)
         {
            ...
            else if(this._data && Object(this._data).hasOwnProperty("backGroundIconUri") && Boolean(Object(this._data).backGroundIconUri))
            {
      ...
      private function updateQuantity(param1:int) : void
      {
         if(param1 == 0)
         {
            if(Boolean(this._quantitySprite) && Boolean(this._quantitySprite.parent))
            {
      ...
      override public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case param1 is MouseDoubleClickMessage:
               _loc2_ = LinkedCursorSpriteManager.getInstance().getItem(DRAG_AND_DROP_CURSOR_NAME);
               if(Boolean(_loc2_) && _loc2_.data is SlotDragAndDropData)
               {
                  LinkedCursorSpriteManager.getInstance().removeItem(DRAG_AND_DROP_CURSOR_NAME);
                  KernelEventsManager.getInstance().processCallback(BeriliaHookList.DropEnd,SecureCenter.secure(SlotDragAndDropData(_loc2_.data).currentHolder));
               }
      ...
      private function onAddedToStage(param1:Event) : void
      {
         ...
         if(Boolean(_loc2_) && Boolean(this._icon))
         {
...
{
   public function DragSprite(param1:BitmapData)
   {
      super();
      alpha = 0.8;
