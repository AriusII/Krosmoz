package com.ankamagames.berilia.components
{
   ...
   public class EntityDisplayer extends GraphicContainer implements UIComponent, IRectangle
   {
      ...
      private var _waitingForEquipement:Array = new Array();
      ...
      public function set look(param1:*) : void
      {
         ...
         if(Boolean(_loc2_) && Boolean(this._lookUpdate))
         {
      ...
      public function equipCharacter(param1:Array, param2:int = 0) : void
      {
         ...
         else if(!this._entity && Boolean(param1.length))
         {
      ...
      private function onCharacterReady(param1:Event) : void
      {
         ...
         var _loc5_:DisplayInfoSprite = null;
         ...
         if(this._view != null)
         {
            _loc5_ = TiphonSprite(this._entity).getDisplayInfoSprite(this._view);
            if(_loc5_ != null)
            {
               TiphonSprite(this._entity).look.setScales(1,1);
               TiphonSprite(this._entity).setView(this._view);
               _loc3_ = this._entity.width / this._entity.height;
               if(this._entity.width > this._entity.height)
               {
                  this._entity.height = width / _loc3_ * this._scale;
                  this._entity.width = width * this._scale;
               }
               else
               {
                  this._entity.width = height * _loc3_ * this._scale;
                  this._entity.height = height * this._scale;
               }
               _loc4_ = TiphonSprite(this._entity).getBounds(this);
               this._entity.x = (width - this._entity.width) / 2 - _loc4_.left + this.xOffset;
               this._entity.y = (height - this._entity.height) / 2 - _loc4_.top + this.yOffset;
               _loc6_ = _loc5_.width / _loc5_.height;
               _loc7_ = width / height < _loc5_.width / _loc5_.height ? width / _loc5_.getRect(this).width : height / _loc5_.getRect(this).height;
               this._entity.height *= _loc7_;
               this._entity.width *= _loc7_;
               this._entity.x -= _loc5_.getRect(this).x;
               this._entity.y -= _loc5_.getRect(this).y;
            }
         }
         else
         {
