   ...
   public dynamic class ReadOnlyObject extends Proxy implements Secure
   {
      ...
      public function ReadOnlyObject(param1:Object, param2:Object)
      {
         ...
         if(!(this._properties && (param1 is Array || param1 is Vector.<*>)))
         {
            ...
            if(Boolean(_createdObjectProperties[this._getQualifiedClassName]) || this._getQualifiedClassName == "Object")
            {
               return;
            }
         ...
         switch(QName(name).localName)
         {
            case "toString":
               try
               {
                  if(!this._testHaveOP)
                  {
                     haveOP = Boolean(this._object.hasOwnProperty("toString"));
                  }
               }
               catch(e:Error)
               {
      ...
      {
         if(this._object is ICustomSecureObject && Boolean(DescribeTypeCache.getTags(this._object)[param1.localName]["Untrusted"]))
         {
