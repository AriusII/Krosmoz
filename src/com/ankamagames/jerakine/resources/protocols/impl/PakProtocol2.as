package com.ankamagames.jerakine.resources.protocols.impl
{
   ...
   public class PakProtocol2 extends AbstractProtocol implements IProtocol
   {
      ...
      public function load(param1:Uri, param2:IResourceObserver, param3:Boolean, param4:ICache, param5:Class, param6:Boolean) : void
      {
         ...
         index = _indexes[uri.path][uri.subPath];
         if(!index)
         {
            if(observer)
            {
      ...
      private function initStream(param1:Uri) : FileStream
      {
         ...
         while(Boolean(_loc3_) && Boolean(_loc3_.exists))
         {
            _loc6_ = new FileStream();
            _loc6_.open(_loc3_,FileMode.READ);
            _loc7_ = int(_loc6_.readUnsignedByte());
            _loc8_ = int(_loc6_.readUnsignedByte());
            if(_loc7_ != 2 || _loc8_ != 1)
            {
               return null;
            }
            _loc6_.position = _loc3_.size - 24;
            _loc9_ = uint(_loc6_.readUnsignedInt());
            _loc10_ = uint(_loc6_.readUnsignedInt());
            _loc11_ = uint(_loc6_.readUnsignedInt());
            _loc12_ = uint(_loc6_.readUnsignedInt());
            _loc13_ = uint(_loc6_.readUnsignedInt());
            _loc14_ = uint(_loc6_.readUnsignedInt());
            ...
            while(_loc17_ < _loc14_)
            {
               ...
               if(_loc15_ == "link")
               {
                  _loc21_ = int(_loc2_.path.lastIndexOf("/"));
                  if(_loc21_ != -1)
                  {
            ...
            while(_loc17_ < _loc12_)
            {
               ...
               _loc19_ = int(_loc6_.readInt());
               _loc20_ = int(_loc6_.readInt());
               _loc4_[_loc18_] = {
                  "o":_loc19_ + _loc9_,
                  "l":_loc20_,
                  "stream":_loc6_
