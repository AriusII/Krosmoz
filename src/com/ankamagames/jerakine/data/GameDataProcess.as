package com.ankamagames.jerakine.data
{
   ...
   public class GameDataProcess
   {
      ...
      public function query(param1:String, param2:Function) : Vector.<uint>
      {
         ...
         var _loc4_:* = this._searchFieldType[param1];
         var _loc5_:Function = this.getReadFunction(_loc4_);
         var _loc6_:uint = uint(this._searchFieldCount[param1]);
      ...
      public function queryEquals(param1:String, param2:*) : Vector.<uint>
      {
         ...
         var _loc5_:uint = uint(this._searchFieldCount[param1]);
         Object(this._stream).position = this._searchFieldIndex[param1];
         var _loc6_:* = this._searchFieldType[param1];
      ...
      private function getSortFunction(param1:*, param2:*) : Function
      {
         ...
         maxFieldIndex = uint(fieldNames.length);
      ...
      private function buildSortIndex(param1:String) : void
      {
         ...
         var _loc12_:uint = 0;
         if(Boolean(this._sortIndex[param1]) || !this._searchFieldIndex[param1])
         {
            return;
         }
         var _loc2_:Vector.<uint> = new Vector.<uint>();
         var _loc3_:uint = uint(this._searchFieldCount[param1]);
         Object(this._stream).position = this._searchFieldIndex[param1];
         var _loc4_:Dictionary = new Dictionary();
         this._sortIndex[param1] = _loc4_;
         var _loc5_:* = this._searchFieldType[param1];
      ...
      private function buildI18nSortIndex(param1:String) : void
      {
         ...
         while(_loc5_++ < _loc3_)
         {
            _loc6_ = uint(this._stream.readInt());
            _loc7_ = this._stream.readInt() * 0.25;
            if(_loc7_)
            {
               _loc8_ = uint(I18nFileAccessor.getInstance().getOrderIndex(_loc6_));
               _loc9_ = 0;
               while(_loc9_ < _loc7_)
               {
      ...
      private function parseStream() : void
      {
         ...
         var _loc1_:int = int(this._stream.readInt());
         var _loc2_:uint = Object(this._stream).position + _loc1_ + 4;
         while(_loc1_)
         {
            _loc3_ = uint(this._stream.bytesAvailable);
            ...
            _loc1_ -= _loc3_ - this._stream.bytesAvailable;
         }
      }
   }
}
