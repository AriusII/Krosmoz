package com.ankamagames.jerakine.replay
{
   ...
   public class LogFrame implements Frame
   {
      ...
      public function LogFrame(param1:Boolean)
      {
         ...
         if(Boolean(maxFile) && allowLogUpload)
         {
            ...
            switch(true)
            {
               ...
               case param1 is MouseMessage:
                  ...
                  else
                  {
                     ...
                     if(_loc2_ != null)
                     {
                        while(Boolean(_loc2_.parent) && !(_loc2_ is ICustomUnicNameGetter))
                        {
      ...
      private function getClassIndex(param1:String) : int
      {
         ...
         fieldList = new Array();
         desc = DescribeTypeCache.typeDescription(getDefinitionByName(className) as Class);
         for each(variable in desc..factory..variable)
         {
            fieldList[varCount] = new LogClassField(§§findproperty(XMLList),this.getClassIndex(variable.@type.toString()),this.getStringIndex(variable.@name.toString()).XMLList(variable..metadata.(@name == "Transient")).length());
            varCount++;
         }
