package com.ankamagames.jerakine.types.positions
{
   ...
   public class WorldPoint implements IDataCenter
   {
      ...
      protected function setFromMapId() : void
      {
         ...
         if((this._x & 0x0100) == 256)
         {
            this._x = -(this._x & 0xFF);
         }
         if((this._y & 0x0100) == 256)
         {
      ...
      protected function setFromCoords() : void
      {
         ...
         var _loc1_:uint = uint(this._worldId & 0x0FFF);
         var _loc2_:uint = uint(Math.abs(this._x) & 0xFF);
         if(this._x < 0)
         {
            _loc2_ |= 256;
         }
         var _loc3_:uint = uint(Math.abs(this._y) & 0xFF);
         if(this._y < 0)
         {
            _loc3_ |= 256;
         }
