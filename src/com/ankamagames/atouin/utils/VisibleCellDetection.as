      ...
      public static function detectCell(param1:Boolean, param2:Map, param3:WorldPoint, param4:Frustum, param5:WorldPoint) : PartialDataMap
      {
            ...
            for each(_loc25_ in _loc23_.cells)
            {
               ...
               for each(_loc26_ in _loc25_.elements)
               {
                  if(_loc26_.elementType == ElementTypesEnum.GRAPHICAL)
                  {
                     ...
                     if(Boolean(_loc12_) && _loc12_ is NormalGraphicalElementData)
                     {
                        ...
                        _loc8_ += _loc27_.origin.y + _loc27_.size.y;
                        _loc18_.push(_loc27_.gfxId);
                     }
                     else
                     {
                        _loc8_ += Math.abs(_loc20_);
                     }
                  }
               }
               if(!_loc8_)
               {
                  _loc8_ = int(AtouinConstants.CELL_HEIGHT);
               }
               if(_loc11_ == 100000)
               {
                  _loc11_ = 0;
               ...
               {
                  _loc9_ = int(AtouinConstants.CELL_WIDTH);
               }
         ...
         while(_loc10_ < _loc17_.length)
         {
            if(_loc17_[_loc10_])
            {
               ...
               if((Boolean(_loc16_)) && _loc16_.intersects(_loc15_) == param1)
               {
