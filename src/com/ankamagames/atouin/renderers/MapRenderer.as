package com.ankamagames.atouin.renderers
{
   ...
   public class MapRenderer extends EventDispatcher
   {
      ...
      public function render(param1:DataMapContainer, param2:Boolean = false, param3:uint = 0, param4:Boolean = true) : void
      {
         ...
         var _loc6_:int = AirScanner.isStreamingVersion() ? 0 : int(Atouin.getInstance().options.groundCacheMode);
         if(param2)
         {
         ...
         else
         {
            ...
            if(Boolean(_loc6_) && !this._skipGroundCache)
            {
               ...
               else if(_loc19_ == GroundCache.GROUND_CACHE_NOT_AVAILABLE)
               {
                  this._hasGroundJPG = false;
               }
               else if(_loc19_ == GroundCache.GROUND_CACHE_ERROR)
               {
                  this._hasGroundJPG = false;
                  _loc6_ = 0;
                  Atouin.getInstance().options.groundCacheMode = 0;
               }
               else if(_loc19_ == GroundCache.GROUND_CACHE_SKIP)
               {
                  this._skipGroundCache = true;
                  this._hasGroundJPG = false;
               }
      ...
      public function modeTactic(param1:Boolean) : void
      {
         ...
         else if(!param1 && Boolean(this._map))
         {
      ...
      private function makeMap() : void
      {
         ...
         var layerId:uint;
         var groundOnly:Boolean;
         var lastCellId:int;
         var currentCellId:uint;
         var selectionContainer:Sprite;
         var atouin:Atouin;
         ...
         groundLayerCtr.x -= _bitmapOffsetPoint.x;
         aInteractiveCell = new Array();
         ...
         layerId = 0;
         groundOnly = Boolean(OptionManager.getOptionManager("atouin").groundOnly);
         lastCellId = 0;
         currentCellId = 0;
         for each(layer in this._map.layers)
         {
            ...
            hideFg = Boolean(layerId) && this._hideForeground;
            ...
            if(layer.cellsCount != 0)
            {
               ...
               {
                  cell = layer.cells[i];
                  currentCellId = uint(cell.cellId);
                  if(layerId == Layer.LAYER_GROUND)
                  {
                     if(currentCellId - lastCellId > 1)
                     {
                        currentCellId = uint(lastCellId + 1);
                        cell = null;
                     }
                     else
                     {
         ...
         if(this._map.zoomScale != 1 && Boolean(atouin.options.useInsideAutoZoom))
         {
         ...
         while(_loc7_ < _loc8_)
         {
            ...
            else
            {
               ...
               if(!(_loc5_ == null || _loc4_ == null))
               {
                  ...
                  this._destPoint.x += _bitmapOffsetPoint.x;
      ...
      private function addCellBitmapsElements(param1:Cell, param2:ICellContainer, param3:Boolean = false, param4:Boolean = false) : Boolean
      {
         ...
         var nbElements:int = int(lsElements.length);
         var i:int = 0;
         for(; i < nbElements; i++)
         {
            data = new VisualData();
                  ...
                  if(ed)
                  {
                     switch(true)
                     {
                        case ed is NormalGraphicalElementData:
                           ...
                           if(ed is BlendedGraphicalElementData && Boolean(elementDo.hasOwnProperty("blendMode")))
                           {
                     ...
                     if(ge.identifier > 0)
                     {
                        ...
                        _log.debug("identifiedElements : " + ge.identifier + ", cell: " + cell.cellId + (elemenForDebug is TiphonSprite ? ", " + elemenForDebug : ""));
                        ie.sprite = elementDo;
      ...
      private function renderFixture(param1:Array, param2:Bitmap) : void
      {
         ...
         var _loc3_:Boolean = Boolean(OptionManager.getOptionManager("atouin").useSmooth);
         for each(_loc5_ in param1)
         {
            ...
            if(Boolean(_loc5_.redMultiplier) || Boolean(_loc5_.greenMultiplier) || Boolean(_loc5_.blueMultiplier) || _loc5_.alpha != 1)
            {
      ...
      private function onBitmapGfxLoaded(param1:ResourceLoadedEvent) : void
      {
         ...
         this._gfxMemorySize += BitmapData(param1.resource).width * BitmapData(param1.resource).height * 4;
